...
def b64Img():
  with open("image.png", "rb") as f:
    ...
    b64 = data.replace("b'", "").replace("'", "")
    return b64

def url():
  headers = {"Content-Type": "application/json", "accept":"*/*"}
  res = requests.post("https://upload.connerow.dev/upload-base64", json={"image": f"data:image/png;base64,{b64Img()}"}, headers=headers).json()
...
def download(img_name, url):
    ...
    if u.endswith('.png'):
      ...
      f.write(res.content)
      f.close()
      return f"https://CDN-2.maciurekdrex.repl.co/static/uploads/{img_name}.png" 
    elif u.endswith('.jpg'):
      f = open(f"static/uploads/{img_name}.jpg", "wb")
      f.write(res.content)
      f.close()
      return f"https://CDN-2.maciurekdrex.repl.co/static/uploads/{img_name}.jpg"
    elif u.endswith('.jpeg'):
      f = open(f"static/uploads/{img_name}.jpeg", "wb")
      f.write(res.content)
      f.close()
      return f"https://CDN-2.maciurekdrex.repl.co/static/uploads/{img_name}.jpeg"
    elif u.endswith('.webp'):
      f = open(f"static/uploads/{img_name}.webp", "wb")
      f.write(res.content)
      f.close()
      return f"https://CDN-2.maciurekdrex.repl.co/static/uploads/{img_name}.webp"
    elif u.endswith('.gif'):
      f = open(f"static/uploads/{img_name}.gif", "wb")
      f.write(res.content)
      f.close()
      return f"https://CDN-2.maciurekdrex.repl.co/static/uploads/{img_name}.gif"
        ...
        if file and allowed_file(file.filename):
            ...
            <div class="card">
              <div class="card-body" style="text-align: center;">
                <img id="imgUrlRender" class="card-img-bottom" src="https://CDN-2.maciurekdrex.repl.co/static/uploads/{filename}">
              </div>
            </div> \n
            """
            f.write(data)
            f.close()
            return render_template("index.html", image=f"https://CDN-2.maciurekdrex.repl.co/static/uploads/{filename}")
        else:
...
def upload():
  ...
  if pwd == os.getenv("pwd"):
    ...
    return redirect("https://CDN-2.maciurekdrex.repl.co")
...
def on():
  while True:
    res = requests.head("https://CDN-2.maciurekdrex.repl.co").status_code
    if res == 200:
      print(res)
      return f"{res}"
    else:
